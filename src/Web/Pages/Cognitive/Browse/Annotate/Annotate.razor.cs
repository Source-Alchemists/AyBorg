...
namespace AyBorg.Web.Pages.Cognitive.Browse.Annotate;
...
public partial class Annotate : ComponentBase
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (firstRender)
        {
            ...
            {
                _selectedImageNames = StateService.CognitiveState.Annotation.SelectedImageNames;
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        try
        {
            ...
            {
                RectangleLayer layer = await AnnotationManagerService.GetRectangleLayer(new AnnotationManagerService.GetRectangleLayerParameters(ProjectId, ImageName, ProjectType.ObjectDetection, id));
    ...
    private async Task SelectedImageNumberChanged(int value)
    {
        ...
        await StateService.SetNetStateAsync(StateService.CognitiveState with { Annotation = StateService.CognitiveState.Annotation with { SelectedImageIndex = index } });
        NavigationManager.NavigateTo($"cognitive/browse/{ProjectId}/annotate/{_selectedImageNames.ElementAt(index)}");
    }
    ...
    private async ValueTask SaveAnnotationsAsync()
    {
        ...
        {
            await AnnotationManagerService.RemoveAsync(new Services.Cognitive.AnnotationManagerService.RemoveParameters(
                ProjectId,
                ImageName,
                layer.Id
            ));
