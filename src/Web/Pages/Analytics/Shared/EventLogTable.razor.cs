...
public partial class EventLogTable : ComponentBase
{
    ...
    private async void DownloadLogAsCsv()
    {
        ...
        try
        {
            ...
            stringBuilder.AppendLine("Timestamp (UTC);Log Level;Service Name;Event;Event ID;Message");
            foreach (EventLogEntry entry in FilteredEntries)
            {
                stringBuilder.AppendLine($"{entry.Timestamp};{entry.LogLevel};{entry.ServiceUniqueName};{entry.EventName};{entry.EventId};{entry.Message}");
            }
